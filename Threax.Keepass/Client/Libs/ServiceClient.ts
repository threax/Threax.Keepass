...
export class DbStatusInjector {
    ...
    public load(): Promise<DbStatusResult> {
        if (!this.instancePromise) {
            if (this.data) {
                this.instancePromise = Promise.resolve(new DbStatusResult(new hal.HalEndpointClient(this.data, this.fetcher)));
            }
            else {
...
export class DbStatusResult {
    ...
    public static Load(url: string, fetcher: Fetcher): Promise<DbStatusResult> {
...
export class PasswordInfoResult {
    ...
    public getEntry(): Promise<ItemResult> {
        return this.client.LoadLink("GetEntry")
               .then(r => {
                    return new ItemResult(r);
                });
